module.exports.function = function getOld (name) {
  ...
  if (totalcount <= 0) {
    ...
    const url = config.get('urlold') + '?serviceKey=' + key + '&itemName=' + encodeURI(name)
    ...
    if (totalcount >= 1) {
      ...
      var message = "만 65세 이상이라면 " + name + "은(는) 위험해요."
    ...
    // 먹어도 괜찮을 때!!
      ...
      var message = "만 65세 이상이 " + name + "을(를) 먹어도 괜찮아요."
